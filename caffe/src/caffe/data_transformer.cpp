...
template<typename Dtype>
void DataTransformer<Dtype>::TransformBoth(const std::vector<cv::Mat>& cv_img_label,
	   Blob<Dtype>* transformed_data_blob, Blob<Dtype>* transformed_label_blob) {
  CHECK(cv_img_label.size() == 2) << "Input must contain image and label image.";
  
  const int crop_size = param_.crop_size();
  const int img_channels = cv_img_label[0].channels();
  int img_height = cv_img_label[0].rows;
  int img_width = cv_img_label[0].cols;
  const int label_channels = cv_img_label[1].channels();
  int label_height = cv_img_label[1].rows;
  int label_width = cv_img_label[1].cols;

  // Check dimensions.
  const int channels = transformed_data_blob->channels();
  const int height = transformed_data_blob->height();
  const int width = transformed_data_blob->width();
  const int num = transformed_data_blob->num();
  ...
  // check that img and label are same size
  CHECK_EQ(img_height, label_height);
  CHECK_EQ(img_width, label_width);

  CHECK(cv_img_label[0].depth() == CV_8U) << "Image data type must be unsigned byte";
  CHECK(cv_img_label[1].depth() == CV_8U) << "Label data type must be unsigned byte";
  ...
  cv::Mat cv_cropped_img = cv_img_label[0];
  cv::Mat cv_cropped_label = cv_img_label[1];
  // transform to double, since we will pad mean pixel values
  cv_cropped_img.convertTo(cv_cropped_img, CV_64F);
  // Check if we need to pad img to fit for crop_size
  // copymakeborder
  ...
  if (pad_height > 0 || pad_width > 0) {
    ...
    cv::copyMakeBorder(cv_cropped_label, cv_cropped_label, 0, pad_height, 
          0, pad_width, cv::BORDER_CONSTANT, 
  		       cv::Scalar(255)); // really this should be ignore_label
    // update height/width
    img_height   = cv_cropped_img.rows;
    img_width    = cv_cropped_img.cols;
  ...
  int top_index;
  for (int h = 0; h < height; ++h) {
    const double* img_ptr = cv_cropped_img.ptr<double>(h);
