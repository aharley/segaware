...
void Im2parityLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  has_ignore_label_ = this->layer_param_.im2parity_param().has_ignore_label();
  if (has_ignore_label_) {
    ignore_label_ = this->layer_param_.im2parity_param().ignore_label();
  }

}

template <typename Dtype>
void Im2parityLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void Im2parityLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    im2parity_cpu(bottom_data + n * bottom_dim_, channels_,
...
template <typename Dtype>
void Im2parityLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  NOT_IMPLEMENTED;
}

#ifdef CPU_ONLY
STUB_GPU(Im2parityLayer);
#endif

INSTANTIATE_CLASS(Im2parityLayer);
REGISTER_LAYER_CLASS(Im2parity);
