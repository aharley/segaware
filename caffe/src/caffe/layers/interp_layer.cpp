...
void InterpLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const InterpParameter& param = this->layer_param_.interp_param();
  int num_specs = 0;
  num_specs += param.has_zoom_factor();
  num_specs += param.has_shrink_factor();
  num_specs += param.has_height() && param.has_width();
  CHECK_EQ(num_specs, 1) << "Output dimension specified either by "
			 << "zoom factor or shrink factor or explicitly";
  pad_beg_ = param.pad_beg();
  pad_end_ = param.pad_end();
...
void InterpLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  const InterpParameter& param = this->layer_param_.interp_param();
  if (param.has_zoom_factor()) {
    const int zoom_factor = param.zoom_factor();
  ...
  else if (param.has_shrink_factor()) {
    const int shrink_factor = param.shrink_factor();
  ...
  else if (param.has_height() && param.has_width()) {
    height_out_  = param.height();
    width_out_  = param.width();
  }
  else {
