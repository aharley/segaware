...
void NormConvMeanfieldLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  vector<int> normconv_spatial_param_shape(1, 4); // 0,1 for rgbxy, 2,3 for xy
  vector<int> normconv_basic_param_shape(1, 2); // 0 for rgbxy, 1 for xy
  int num_spatial_axes_ = 2;
  int num_convolutions_ = 2;
  // Setup kernel dimensions (kernel_shape_).
  kernel_shape_.Reshape(normconv_spatial_param_shape);
  int* kernel_shape_data = kernel_shape_.mutable_cpu_data();
  const int num_kernel_dims = meanfield_param.kernel_size_size();
  CHECK(num_kernel_dims == num_convolutions_)
    << "kernel_size must be specified once for each norm_conv "
    << "(kernel_size specified " << num_kernel_dims << " times).";
  for (int i = 0; i < num_spatial_axes_*num_convolutions_; ++i) {
    kernel_shape_data[i] = meanfield_param.kernel_size(i/num_convolutions_);
  ...
  CHECK(num_stride_dims == 0 ||	num_stride_dims == num_convolutions_)
    << "stride must be specified once for each norm_conv "
    << "(stride specified " << num_stride_dims << " times).";
  const int kDefaultStride = 1;
  for (int i = 0; i < num_spatial_axes_*num_convolutions_; ++i) {
    stride_data[i] = (num_stride_dims == 0) ? kDefaultStride :
      meanfield_param.stride(i/num_convolutions_);
  ...
  CHECK(num_remove_center_dims == 0 || num_remove_center_dims == num_convolutions_)
  ...
  for (int i = 0; i < num_convolutions_; ++i) {
    remove_center_data[i] = (num_remove_center_dims == 0) ? kDefaultRemoveCenter :
      meanfield_param.remove_center(i);
  }
  ...
  CHECK(num_remove_bounds_dims == 0 || num_remove_bounds_dims == num_convolutions_)
  ...
  for (int i = 0; i < num_convolutions_; ++i) {
    remove_bounds_data[i] = (num_remove_bounds_dims == 0) ? kDefaultRemoveBounds :
      meanfield_param.remove_bounds(i);
  }
  ...
  // top[1]->Reshape(num_, 2, height_, width_);
  ...
  if (this->blobs_.size() > 0) {
    ...
    // maybe i want to do this (later)
    // // blobs_[3] - spatial conv weights
    // // blobs_[4] - spatial conv biases
    // // blobs_[5] - bilat conv weights
    // // blobs_[6] - bilat conv biases
    ...
    for (int i = 0; i < channels_; i++) {
      ...
      // to simplify, let's set these to ones
      this->blobs_[1]->mutable_cpu_data()[i * channels_ + i] = 1;
    }
    // fclose(pFile);
  ...
  xy_split_layer_bottom_vec_.clear();
  xy_split_layer_bottom_vec_.push_back(bottom[3]);
  xy_split_layer_top_vec_.clear();
  xy_split_layer_out_blobs_.resize(num_iterations_);
  for (int i = 0; i < num_iterations_; i++) {
    xy_split_layer_out_blobs_[i].reset(new Blob<Dtype>());
    xy_split_layer_top_vec_.push_back(xy_split_layer_out_blobs_[i].get());
  }
  LayerParameter xy_split_layer_param;
  xy_split_layer_.reset(new SplitLayer<Dtype>(xy_split_layer_param));
  xy_split_layer_->SetUp(xy_split_layer_bottom_vec_, xy_split_layer_top_vec_);
  ...
  for (int i = 0; i < num_iterations_; ++i) {
    ...
    meanfield_iterations_[i].reset(new NormConvMeanfieldIteration<Dtype>());
    meanfield_iterations_[i]->OneTimeSetUp(
        ...
        xy_split_layer_out_blobs_[i].get(), // xy
  ...
  // LOG(INFO) << ("NormConvMeanfieldLayer initialized.");
}

template <typename Dtype>
void NormConvMeanfieldLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
	// Do nothing.
}
 ...
 * bottom[3] - position images
 *
 * top[0] - Output of the mean field inference (not normalized).
 */
template <typename Dtype>
void NormConvMeanfieldLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  xy_split_layer_bottom_vec_[0] = bottom[3];
  xy_split_layer_->Forward(xy_split_layer_bottom_vec_, xy_split_layer_top_vec_);
 ...
 */
template<typename Dtype>
void NormConvMeanfieldLayer<Dtype>::Backward_cpu(
  ...
  xy_split_layer_->Backward(xy_split_layer_top_vec_, split_layer_propagate_down, xy_split_layer_bottom_vec_);
...
INSTANTIATE_CLASS(NormConvMeanfieldLayer);
REGISTER_LAYER_CLASS(NormConvMeanfield);
}  // namespace caffe
