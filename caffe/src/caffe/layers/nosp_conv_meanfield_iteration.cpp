...
void NospConvMeanfieldIteration<Dtype>::OneTimeSetUp(
  ...
  if (this->blobs_.size() > 0) {
    LOG(INFO) << "NospConvMeanfield iteration skipping parameter initialization.";
  } else {
    blobs_.resize(3);
    blobs_[0].reset(new Blob<Dtype>(1, 1, channels_, channels_)); // bilateral kernel weight
    blobs_[1].reset(new Blob<Dtype>(1, 1, channels_, channels_)); // compatibility transform matrix
  }
  ...
  rgbxy_conv_bottom_vec_.push_back(&prob_); // image
 ...
 */
template <typename Dtype>
void NospConvMeanfieldIteration<Dtype>::PrePass(
 ...
 */
template <typename Dtype>
void NospConvMeanfieldIteration<Dtype>::Forward_cpu() {
...
template<typename Dtype>
void NospConvMeanfieldIteration<Dtype>::Backward_cpu() {
  //---------------------------- Add unary gradient --------------------------
...
INSTANTIATE_CLASS(NospConvMeanfieldIteration);
