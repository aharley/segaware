...
template <typename Dtype>
class XYFillerTest : public ::testing::Test {
 protected:
  XYFillerTest()
      : blob_(new Blob<Dtype>(3, 2, 4, 5)),
        filler_param_() {
    // filler_param_.set_value(10.);
    filler_.reset(new XYFiller<Dtype>(filler_param_));
    filler_->Fill(blob_);
  }
  virtual ~XYFillerTest() { delete blob_; }
  Blob<Dtype>* const blob_;
  FillerParameter filler_param_;
  shared_ptr<XYFiller<Dtype> > filler_;
};

TYPED_TEST_CASE(XYFillerTest, TestDtypes);

TYPED_TEST(XYFillerTest, TestFill) {
  EXPECT_TRUE(this->blob_);
  const TypeParam* data = this->blob_->cpu_data();
  ...
  for (int n=0; n < num; n++) {
	...
	for (int i = 0; i < width; i++) {
	  if (c==0)
	    EXPECT_EQ(data[i+j*width+c*height*width+n*c*height*width],i);
	  else
	    EXPECT_EQ(data[i+j*width+c*height*width+n*c*height*width],j);
	}
      }
    }
  }
